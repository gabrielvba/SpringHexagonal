...
import com.acc.oss.storeReact.infrastructure.adapter.in.rest.dto.Product;
import com.acc.oss.storeReact.infrastructure.adapter.in.transform.TransformProductDtoToModel;
...
public class ProductController {
	...
	@PostMapping
	public Mono<ResponseEntity<com.acc.oss.storeReact.domain.model.Product>> criarProduto(@RequestBody Product product) {
	    log.info("Create Product: {}", product);

	    return Mono.just(product)
	            .map(TransformProductDtoToModel::execute)
	            .flatMap(productModel -> createOrderUserCase.criarProduto(productModel)
	...
	@GetMapping
	public Flux<com.acc.oss.storeReact.domain.model.Product> listarProdutos() {
		log.info("List products");
		return createOrderUserCase.listarProdutos();
	}

	@PutMapping("/{id}")
	public Mono<ResponseEntity<com.acc.oss.storeReact.domain.model.Product>> atualizarProduto(@PathVariable Long id, @RequestBody Product product) {
	    log.info("Update Product: {}", product);

	    return Mono.fromCallable(() -> TransformProductDtoToModel.execute(product))
	            .flatMap(productModel -> createOrderUserCase.atualizarProduto(id, productModel)
	...
	public Mono<ResponseEntity<Object>> deletarProduto(@PathVariable Long id) {
	    return createOrderUserCase.deletarProduto(id)
